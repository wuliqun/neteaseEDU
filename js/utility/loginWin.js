(function(_){
	...
	function LoginWin(options){
		...
		this.head = $s(this.body,'title')[0];
		this.close = $s(this.body,'close-login')[0];
		this.form = $t(this.body,'form')[0];
		this.iptUser = this.form[0];
		this.iptPass = this.form[1];
		this.submit = this.form[2];
		this.logfail = $s(this.body,'logfail')[0];
		this.userError = $s(this.body,'user-err')[0];
		this.passError = $s(this.body,'pass-err')[0];
		_.extend(this,options);
		this._init();
		this._initEvents();
		
	}
	_.extend(LoginWin.prototype,_);
	_.extend(LoginWin.prototype,{
		...
		_validate:function(user,pass){
			...
			if(!userReg.test(user)){
				util.addClass(this.iptUser,'invalid');
				util.addClass(this.userError,'z-act');
				if(!user){					
			...
			if(!passReg.test(pass)){
				util.addClass(this.iptPass,'invalid');
				util.addClass(this.passError,'z-act');
				if(!pass){
		...
		_focus:function(ipt){
			util.delClass(ipt,'invalid');
			switch(ipt){
			case this.iptUser:
				util.delClass(this.userError,'z-act');
				break;
			case this.iptPass:
				util.delClass(this.passError,'z-act');
				break;
			}
		},
		//登录 
		_login:function(e){
			...
			if(this._validate(user,pass)){
				var md5User=util.md5(user),
					md5Pass=util.md5(pass);
		...
		_placeholder:function(){
			...
			if(!('placeholder' in input)){
				util.addClass(this.iptUser,'placeholder');
				...
				util.addEvent(this.iptUser,'focus',this._holderOff.bind(this,this.iptUser));
				util.addEvent(this.iptUser,'blur',this._holderOn.bind(this,this.iptUser));
				util.addEvent(input,'focus',this._holderOff.bind(this,this.iptPass,input));
				util.addEvent(this.iptPass,'blur',this._holderOn.bind(this,this.iptPass,input));
			}
		},
		//显示placeholder
		...
		_initEvents:function(){
			...
			util.addEvent(this.form,'submit',this._login.bind(this));
			util.addEvent(this.iptUser,'focus',this._focus.bind(this,this.iptUser));
			util.addEvent(this.iptPass,'focus',this._focus.bind(this,this.iptPass));
			this._placeholder();
		}
	});
