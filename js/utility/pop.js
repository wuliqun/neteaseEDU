(function(_){
	...
	function Pop(){
		...
		this.sd = util.template.parse(tpl);
		this._initEvents();
	}
	_.extend(Pop.prototype,_);
	_.extend(Pop.prototype,{
		_layout:_.html2node(body),
		_init:function(options){
			this.body.innerHTML=util.template.merge(this.sd,options);
		},
		...
		_initEvents:function(){
			...
			var self = this,
				timer;
			util.addEvent(this.body,'mouseout',function(){
				timer = setTimeout(self.hide.bind(self),0);
			});
			//立即触发了mouseover,清除定时器
			util.addEvent(this.body,'mouseover',function(){
				clearInterval(timer);
			});
		}
	});
