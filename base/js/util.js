...
var util = {
    ...
    data:function(ele,name,value){
        if(typeof value === 'undefined'){
            if(ele.dataset){
                ...
                return ele.getAttribute('data-'+name,value);
            }
        }else{
            if(ele.dataset){
    ...
    //deep表示是否覆盖a1已存在属性,默认不覆盖
    extend:function(a1,a2,deep){
        if(!a2) return;
        deep = !!deep;
        for(var a in a2){
            if(deep || typeof a1[a] === 'undefined'){
                a1[a] = a2[a];
            }
        }
    },
...
//md5函数,引自https://github.com/wbond/md5-js/blob/master/md5.js
util.md5 = (function() {
